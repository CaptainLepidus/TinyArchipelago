...
function hexScale(a, k) {
    return Hex(a.q * k, a.r * k, a.s * k);
}

function hexRotateLeft(a) {
    return Hex(-a.s, -a.q, -a.r);
}

function hexRotateRight(a) {
...
function hexDiagonalNeighbor(hex, direction) {
    return hexAdd(hex, hex_diagonals[direction]);
}
...
var EVEN = 1;
var ODD = -1;
function qOffsetFromCube(offset, h) {
    var col = h.q;
    var row = h.r + Math.trunc((h.q + offset * (h.q & 1)) / 2);
    return OffsetCoord(col, row);
}

function qOffsetToCube(offset, h) {
    var q = h.col;
...
function polygonCorners(layout, h) {
    ...
    for (var i = 0; i < 6; i++)
    {
        ...
        corners.push(Point(offset.x, offset.y));
    }
    return corners;
}

function pointsToPairs(pointArray) {
    var arr = [];
...
{
    equal_hex("hexAdd", Hex(4, -10, 6), hexAdd(Hex(1, -3, 2), Hex(3, -7, 4)));
    equal_hex("hexSubtract", Hex(-2, 4, -2), hexSubtract(Hex(1, -3, 2), Hex(3, -7, 4)));
}

function test_hex_direction()
{
    equal_hex("hexSubtract", Hex(0, -1, 1), hexSubtract(2));
}
